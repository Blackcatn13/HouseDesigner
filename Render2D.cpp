...
void Render2D::mousePressEvent(QMouseEvent *event)
{
    ...
    {
    case INSERTING: qDebug() << "INSERTING";
        switch(t)
        {
        case WALL:
        ...
        break;
    case SELECTING: qDebug() << "SELECTING";
                    qDebug() << round(wx)<< round(wz);
                    FirstClickObject(wx,wz);
                    Picked = scenary->getPickedObject(round(wx),round(wz),0.5,indx);
                    if (indx != -1)
                    {
                        qDebug() << "pickedModel" << Picked.modelName.c_str() << "type" << Picked.type;
                        qDebug() << "Index" << indx;
                    }else qDebug() << "No detectat";
...
void Render2D::mouseReleaseEvent(QMouseEvent*)
{
    if(clicked)
    {
        ...
        case DELETING:
            break;
        case SELECTING: if(t==OBJECT)

                                 /* for(int i=0; i<scenary->m_ObjectModels.size(); i++)
                                {
...
void Render2D::mouseMoveEvent(QMouseEvent *event, int, int)
{
    ...
    if(wx > 0.0 && wz > 0.0 && wx < scenary->getGridMaxX() && wz < scenary->getGridMaxZ())
    {
        if(clicked)
        {
            ...
            case DELETING:
                break;
            case SELECTING:// if(scenary->getWall2ObjectCollision()==false)
                           //  object.translate(coordenades)
                break;
            }
        }
    }
